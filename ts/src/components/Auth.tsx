...
export function AuthForm(){
    return (<div class="flex flex-col justify-center items-center">
        <JoinHeader class="fill-brand h-25 mb-20"/>
        <div>
            <div class="justify-center items-center flex flex-col">
                ...
                <span class="font-[LeagueSpartan] text-brand text-3xl font-bold mb-4">login with</span>
                <div class="flex gap-4 justify-center items-center">
                    <div class="w-11 h-11 flex justify-center items-center bg-[#5865f2] rounded-sm cursor-pointer" onClick={async () => await supabase.auth.signInWithOAuth({
                        provider: "discord"
                    })}>
                        <img class="w-7" src={discord}></img>
                    </div>
                    <div class="w-11 h-11 bg-[#f6f8fa] rounded-sm flex justify-center items-center gap-2 cursor-pointer" onClick={async () => await supabase.auth.signInWithOAuth({
                        provider: "github"
                    })}>
                        <img class="w-7" src={github_mark}></img>
                    </div>
                </div>
            </div>
